...
// 병원 데이터 관리 페이지 — Supabase clinics 테이블 연동
// 병원 목록 조회, 이름 검색, 인증/활성 토글 제공

import { useEffect, useState, useCallback } from "react";

// Supabase clinics 테이블 행 구조 (필요한 필드만)
interface ClinicRow {
  id: number;
  name: string;
  name_en: string | null;
        ...
        {clinics.map((clinic) => (
          <div
            ...
            className={`bg-white rounded-xl shadow-sm border p-5 ${
              !clinic.is_active ? "border-red-200 bg-red-50/20" : "border-gray-100"
            }`}
          >
            <div className="flex justify-between items-start">
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-500 mt-1">
                  {[clinic.sido_cd_nm, clinic.cl_cd_nm, clinic.phone]
                    .filter(Boolean)
                    .join(" · ") || "-"}
                </p>
                ...
                <div className="flex gap-3 mt-2 text-xs text-gray-400">
                  {clinic.rating_avg != null && (
                    <span>평점: {clinic.rating_avg.toFixed(1)}</span>
                  )}
                  {clinic.review_count != null && (
                    <span>리뷰: {clinic.review_count}건</span>
                  )}
                  <span>
                    최종 수정: {new Date(clinic.updated_at).toLocaleDateString("ko-KR")}
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2 ml-4 shrink-0">
                <button
                  onClick={() =>
                    handleAction(clinic.id, clinic.is_verified ? "unverify" : "verify")
                  }
                  disabled={acting === clinic.id}
                ...
                >
                  {clinic.is_verified ? "인증해제" : "인증"}
                </button>
                <button
                  onClick={() =>
                    handleAction(clinic.id, clinic.is_active ? "deactivate" : "activate")
                  }
                  disabled={acting === clinic.id}
